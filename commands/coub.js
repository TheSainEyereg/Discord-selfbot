...
module.exports = {
	...
	execute(message, args) {
		...
		if (!communities.includes(args[0]) || !args[0]) return Messages.warning(message, "Unknown category!", {description: `Communities: ${communities.join(", ")}.`, timeout: 3000});
		...
		.then(res => {
			...
			if (!coubs) return Messages.error(message, "Error in getting coubs list!", {timeout: 2000});
			const coub = coubs[Math.floor(Math.random()*per_page)].id;
			if (!coub) return Messages.error(message, "Error in getting coub list!", {timeout: 2000});
			...
			.then(res => {
				...
				if (!url || !link) return Messages.error(message, "Error in getting video!", {timeout: 2000});
			...
			.catch(e => {
				return Messages.error(message, "Error in fetching coub info!", {discription: `\`\`\`${e}\`\`\``, timeout: 2000});
			});
		})
		.catch(e => {
			return Messages.error(message, "Error in fetching coubs list!", {discription: `\`\`\`${e}\`\`\``, timeout: 2000});
		});
	}
};